...
public class report extends AppCompatActivity {
    ...
    private TextView monthh, yearr;
    private Map<String, String> nameToId;
    private int currentIndex;
    LinearLayout linearLayoutHorizontal, linearLayoutNames;
    TextView inputKidName;
    ...
    private TextView lastSelectedLevel;
    private int lastSelectedLevelNumber = -1;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        inputKidName = findViewById(R.id.inputKidName);
        monthh = findViewById(R.id.month);
        yearr = findViewById(R.id.year);
    ...
    private void fetchNames() {
        DatabaseReference namesRef = FirebaseDatabase.getInstance().getReference(mainCollection).child("name=id");
...
//                }
                ...
                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                    ...
                    addKidNameTextView(name);
                }

                if (!nameToId.isEmpty() && inputKidName != null) {
                    String firstName = nameToId.keySet().iterator().next();
                    inputKidName.setText(firstName);
                }
            ...
            public void onCancelled(@NonNull DatabaseError databaseError) {
                Toast.makeText(report.this, "فشل في استرجاع الأسماء", Toast.LENGTH_SHORT).show();
            }
        });
    }

//    private void searchAttendance() {
//        final String kidName = inputKidName.getText().toString().trim().toLowerCase();
//        final String year = yearEditText.getText().toString().trim();
//        final String month = monthEditText.getText().toString().trim();
//
//        if (kidName.isEmpty() || year.isEmpty() || month.isEmpty()) {
//            Toast.makeText(this, "الرجاء إدخال كل البيانات المطلوبة", Toast.LENGTH_SHORT).show();
//            return;
//        }
//
//        DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReference(mainCollection).child("dateData").child(year).child(month);
//
...
//                    LinearLayout dayLayout = new LinearLayout(report.this);
//                    dayLayout.setOrientation(LinearLayout.VERTICAL);
//                    dayLayout.setPadding(10, 10, 10, 10);
//                    dayLayout.setBackgroundColor(Color.LTGRAY);
//
...
//
//                    TextView dayTextView = new TextView(report.this);
//                    dayTextView.setText(String.valueOf(day));
//                    dayTextView.setTextSize(20);
//                    dayLayout.addView(dayTextView);
//
//                    Calendar calendar = Calendar.getInstance();
//                    calendar.set(Integer.parseInt(year), Integer.parseInt(month) - 1, day);
...
//                    dayText.append(day).append("\n").append(dayName);
//
//                    if (snapshot.hasChild(dayString)) {
//                        DataSnapshot daySnapshot = snapshot.child(dayString);
//                        boolean kidFound = false;
//                        for (DataSnapshot kidSnapshot : daySnapshot.getChildren()) {
//                            String kidSnapshotName = kidSnapshot.getKey().toLowerCase();
//                            if (kidSnapshotName.contains(kidName)) {
//                                kidFound = true;
//                                String checkIn = kidSnapshot.child("checkIn").getValue(String.class);
//                                String checkOut = kidSnapshot.child("checkOut").getValue(String.class);
//
//                                dayLayout.setBackgroundColor(Color.parseColor("#DFF0D8"));
//
//                                dayText.append("\nحضر");
//
//                                TextView checkInTextView = new TextView(report.this);
//                                checkInTextView.setText(checkIn);
//                                checkInTextView.setTextSize(16);
//                                dayLayout.addView(checkInTextView);
//
//                                TextView checkOutTextView = new TextView(report.this);
//                                checkOutTextView.setText(checkOut);
//                                checkOutTextView.setTextSize(16);
//                                dayLayout.addView(checkOutTextView);
//
//                                try {
...
//
//                                    TextView durationTextView = new TextView(report.this);
//                                    durationTextView.setText(String.format(Locale.ENGLISH, "%02d:%02d:%02d", hours, minutes, seconds));
//                                    durationTextView.setTextSize(16);
//                                    dayLayout.addView(durationTextView);
//                                } catch (Exception e) {
//                                    e.printStackTrace();
//                                }
//                            }
//                        }
//                        if (!kidFound) {
//                            dayLayout.setBackgroundColor(Color.parseColor("#F2DEDE"));
//                            dayText.append("\nغائب");
//                        }
//                    } else {
//                        calendar = Calendar.getInstance();
//                        calendar.set(Integer.parseInt(year), Integer.parseInt(month) - 1, day);
//                        TextView holidayTextView = new TextView(report.this);
//
//                        if (calendar.getTimeInMillis() < System.currentTimeMillis()) {
//                            dayLayout.setBackgroundColor(Color.parseColor("#D9EDF7"));
//                            holidayTextView.setText("إجازة");
//                            dayText.append("\nإجازة");
//                        } else {
//                            dayLayout.setBackgroundColor(Color.parseColor("#FCF8E3"));
//                            holidayTextView.setText("قريبا");
//                            dayText.append("\nقريبا");
//                        }
//                    }
//
//                    dayTextView.setText(dayText.toString());
//
//                    for (int i = 0; i < dayLayout.getChildCount(); i++) {
//                        LinearLayout.LayoutParams params = (LinearLayout.LayoutParams) dayLayout.getChildAt(i).getLayoutParams();
//                        params.weight = 1;
//                        dayLayout.getChildAt(i).setLayoutParams(params);
//                    }
//
//                    linearLayoutHorizontal.addView(dayLayout);
//                }
//            }
//
//            @Override
//            public void onCancelled(@NonNull DatabaseError error) {
//                Toast.makeText(report.this, "حدث خطأ أثناء جلب البيانات", Toast.LENGTH_SHORT).show();
//            }
//        });
//    }

    private void addKidNameTextView(final String kidName) {
        ...
        textView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                kidId = nameToId.get(kidName);
                inputKidName.setText(kidName + "-" + kidId);
                searchAttendance();
                // 8. مراقبة حالة الطالب (In Class)
                DatabaseReference databaseRefer = FirebaseDatabase.getInstance().getReference(mainCollection).child("data").child(kidId);
                databaseRefer.child("isInClass").addValueEventListener(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot snapshot) {
                        ...
                        if (isInClass != null) {
                            TextView inClass=findViewById(R.id.inClass);
                            inClass.setText(isInClass ? "في المؤسسة" : "خارج المؤسسة");
                            inClass.setTextColor(isInClass ? Color.GREEN : Color.RED);
        ...
        linearLayoutNames.addView(textView);
    }
...
//        }
//
//        monthEditText.setText(String.format(Locale.ENGLISH, "%02d", month));
//        yearEditText.setText(String.format(Locale.ENGLISH, "%04d", year));
//
//        searchAttendance();
//    }
}
